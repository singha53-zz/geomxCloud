...
dea_ui <- function(id, datasetName, dataset, response) {
  ...
  shiny::fluidPage(
    shiny::fluidRow(
      ...
      shiny::column(4,
        shiny::radioButtons(ns("deTest"), "Select Method:",
      ...
      shiny::column(6,
        ...
        shiny::radioButtons(ns("fdr"), "Select False Discovery Rate:",
                            c("1%" = 0.01, "5%" = 0.05, "10%" = 0.1),
                            0.1,
                            inline = TRUE)
        )
      ),
    shiny::fluidRow(align = "center",
      shiny::column(6, shinycssloaders::withSpinner(plotly::plotlyOutput(ns("volcanoPlot")))),
      shiny::column(6,
        shiny::div(style = "display:inline-block;vertical-align:top;",
          shiny::fluidRow(
            shiny::column(8,
              dqshiny::autocomplete_input(ns("variable_name"), "Type gene symbol:",
  ...
  shiny::observeEvent(dea_ui_vars$fdr(), {
    shiny::observeEvent(dea_ui_vars$comparison(), {
      shiny::observeEvent(dea_ui_vars$deTest(), {
        ...
        # top <- geomxCloud::generateTopTable(dataset, design, coefNumber = selectedCoef, test = dea_ui_vars$deTest())

        top <- toptables[[dea_ui_vars$comparison()]][[dea_ui_vars$deTest()]]
        subsetTop <- shiny::reactive({
          top = top %>%
            mutate(Significant = ifelse(adj.P.Val < as.numeric(dea_ui_vars$fdr()),
              paste("FDR < ", as.numeric(dea_ui_vars$fdr())), "Not Sig"))
        })

        output$topTable <- shiny::downloadHandler(
          filename = function() {
            paste("Significant_variables_geomxCloud_", datasetName, "_FDR",
        ...
        output$boxplot <- plotly::renderPlotly({
          if (variable_name$selected %in% colnames(dataset)) {
            ...
            geomxCloud::empty_plot("Select point or search for gene symbol.")
          }
        })
        ...
        if (datasetName %in% perform_pathway_analysis) {
          ...
          if(length(all) > 1){
            ...
            enrichedAll <- enrichr_results[[dea_ui_vars$comparison()]][[dea_ui_vars$deTest()]]$all[[dea_ui_vars$fdr()]]
            ...
            updateSliderInput(session, "enrichmentSlider", min = 1, max = length(edgesGsetAll_list),
                              value = min(5, round(length(edgesGsetAll_list)/2, 0)))
          ...
          output$pathwayEnrichment <- renderPlot({
            if(nrow(edgesGsetAll) > 1 & datasetName %in% perform_pathway_analysis){
              ...
              geomxCloud::empty_plot(paste0("No enriched pathways at an FDR = ", dea_ui_vars$fdr()))
            }
          })
          ...
          if(length(up) > 1){
            ...
            enrichedUp <- enrichr_results[[dea_ui_vars$comparison()]][[dea_ui_vars$deTest()]]$up[[dea_ui_vars$fdr()]]
          ...
          if(length(down) > 1){
            ...
            enrichedDown <- enrichr_results[[dea_ui_vars$comparison()]][[dea_ui_vars$deTest()]]$down[[dea_ui_vars$fdr()]]
          ...
          output$drugEnrichmentUp <- renderPlotly({
            if(nrow(edgesPertUp) > 1 & datasetName %in% perform_pathway_analysis){
              ...
              geomxCloud::empty_plot(paste0("No enriched compounds at an FDR = ", dea_ui_vars$fdr()))
            }
          })
          ...
          output$edgesGsetAll <- downloadHandler(
            filename = function() {
              paste("EnrichmentAnalysis_geomxCloud_", datasetName, "_FDR",
                dea_ui_vars$fdr(), "_", Sys.Date(), ".txt", sep="")
            },
            content = function(file) {
          ...
          output$edgesPertUp <- downloadHandler(
            filename = function() {
              paste("Compounds_that_reverse_expression_of_upregulated_variables_geomxCloud_", datasetName, "_FDR",
                dea_ui_vars$fdr(), "_", Sys.Date(), ".txt", sep="")
            },
            content = function(file) {
          ...
          output$edgesPertDown <- downloadHandler(
            filename = function() {
              paste("Compounds_that_reverse_expression_of_downregulated_variables_geomxCloud_", datasetName, "_FDR",
                dea_ui_vars$fdr(), "_", Sys.Date(), ".txt", sep="")
            },
            content = function(file) {
