            ...
            shiny::mainPanel(
              shiny::tabsetPanel(type = "tabs",
                ...
                shiny::tabPanel("Panels",
                  shiny::fluidRow(
                    shiny::column(12, geomxCloud::variable_plot_ui(ns("biomarker_plot")))
                  ),
                  shiny::fluidRow(
                    shiny::column(12,
...
biomarker_discovery_analysis_server <- function(input, output, session, datasets, response, response_var, group_colors, biomarker_discovery_analysis_ui_vars) {
  ...
  shiny::observeEvent(biomarker_discovery_analysis_ui_vars$build(), {
    ...
    output$biomarker_panels = shiny::renderUI({
      geomxCloud::dotplot_ui(ns("biomarker_panels"),
        panel_names = names(singlePanelMods))
    })

    single_panel_ui_vars <- shiny::callModule(module = geomxCloud::dotplot_ui_vars,
      "biomarker_panels")
    shiny::callModule(module = geomxCloud::dotplot_server,
      id = "biomarker_panels",
    ...
    output$ensemble_panel = shiny::renderUI({
      geomxCloud::dotplot_ui(ns("ensemble_panel"),
        panel_names = names(ensemblePanelMods))
    })
    ensemble_panel_ui_vars <- shiny::callModule(module = geomxCloud::dotplot_ui_vars,
      "ensemble_panel")
    shiny::callModule(module = geomxCloud::dotplot_server,
      id = "ensemble_panel",
    ...
    shiny::observeEvent(selected_variable$feature, {
      ...
      if (something_is_there) {
        if (selected_variable$panel %in% names(datasets)){
          print("make plot")
          biomarker_ui_vars <- shiny::callModule(module = geomxCloud::variable_plot_ui_vars,
            "biomarker_plot")
          shiny::callModule(module = geomxCloud::variable_plot_server,
            id = "biomarker_plot",
            response = response,
        ...
        geomxCloud::empty_plot("Please select a point from the dot plots below.")
      }
    })
